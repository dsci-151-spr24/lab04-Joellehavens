...
Each row represents the data for one denny's restaurant. The variables are:
`r names(dennys)`. ***
...
Each row represents the data for one La Quintas Inn. The variables are:
`r names(laquinta)`. ***
...
***There are LaQuinta hotels in several countries but mostly in the US and the Americas.
Dennys are all located in the US.***
...
***We can use the State variable to determine whether a location is in the US or not. Specifically by comparing the state listed to a database of states in the US. ***
...
```
***There are no Dennys locations outside of the US in the data we have. ***
